...
class Bayes(object):
    def __init__(self, tokenizer=None, combiner=None, dataClass=None):
        ...
        if tokenizer is None:
            self._tokenizer = Tokenizer(lower=True) #joel: convert all tokens to lower case
        else:
    ...
    def poolTokens(self, poolName):
        ...
        """
        return [tok for tok, dummy in self.poolData(poolName)]

    def save(self, fname='bayesdata.dat'):
        from cPickle import dump
        import utils
        fp = open(utils.datadir + fname, 'wb')
        dump(self.pools, fp,-1)
        fp.close()

    def load(self, fname='bayesdata.dat'):
        from cPickle import load
        import utils
        fp = open(utils.datadir + fname, 'rb')
        self.pools = load(fp)
        fp.close()
        self.corpus = self.pools['__Corpus__']
        self.dirty = True
    def backup_wordlist(self):
        import shutil
        import utils
        try:
            shutil.copy(utils.datadir+ "bayesdata.dat",utils.datadir+ "bayesdata.bak")
        except: 
            print "Warning : could not backup filter wordlist"
    ...
    def remove_duplicates(self,tokens):
        "remove duplicates in a list of tokens"""
        return list(set(tokens))

    def train(self, pool, item, uid=None,removeDuplicates=True):
        ...
        """
        tokens = list(self.getTokens(item)) #method returns a generator
        if removeDuplicates: tokens = self.remove_duplicates(tokens) #Optionnaly remove duplicates
    ...
    def robinsonFisher(self, probs, dummy):
    ...
    def tokenize(self, obj):
        for match in self.WORD_RE.finditer(obj):
            ...
            if self.lower:
                ...
                token = match.group()
            if token not in utils.common_words: #ignore common words for the filtering, they don't add information
                yield token
